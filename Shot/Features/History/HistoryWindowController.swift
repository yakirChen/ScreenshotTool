...
class HistoryWindowController: NSWindowController {
    ...
    private func setupUI() {
        ...
        let topBar = NSView(
            frame: CGRect(
                x: 0, y: contentView.bounds.height - 40, width: contentView.bounds.width, height: 40
            ))
        ...
        let scrollView = NSScrollView(
            frame: CGRect(
                x: 0, y: 0, width: contentView.bounds.width, height: contentView.bounds.height - 40)
        )
    ...
    private func showPreview(image: NSImage, near screenPoint: CGPoint, title: String) {
        ...
        let scale = min(
            maxPreview.width / image.size.width, maxPreview.height / image.size.height, 1.0)
        ...
        if let label = panel.contentView?.viewWithTag(7001) as? NSTextField {
            ...
            label.frame = CGRect(
                x: 10, y: contentSize.height - 24, width: contentSize.width - 20, height: 16)
        }
...
extension HistoryWindowController: NSTableViewDataSource, NSTableViewDelegate {
    ...
    -> NSView? {
        ...
        let view =
            (tableView.makeView(withIdentifier: id, owner: self) as? HistoryRowView)
            ?? HistoryRowView()
        view.identifier = id
...
final class HistoryRowView: NSTableCellView {
    ...
    func configure(with entry: HistoryManager.HistoryItem) {
        ...
        sizeLabel.stringValue =
            "\(entry.width) × \(entry.height) · \(URL(fileURLWithPath: entry.filePath).lastPathComponent)"
