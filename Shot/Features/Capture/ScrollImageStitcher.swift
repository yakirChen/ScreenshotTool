...
class ScrollImageStitcher {
    ...
    static func stitchPair(top: NSImage, bottom: NSImage) -> NSImage {
        guard let topBitmap = getBitmap(from: top),
              let bottomBitmap = getBitmap(from: bottom)
        else {
        ...
        // 像素转为 point
        let topScale =
            topBitmap.pixelsHigh > 0 ? top.size.height / CGFloat(topBitmap.pixelsHigh) : 1
    ...
    private static func findOverlap(topBitmap: NSBitmapImageRep, bottomBitmap: NSBitmapImageRep)
    -> Int {
        ...
        for bottomRow in 0..<searchRange {
            let bottomRowColors = getRowColors(
                bitmap: bottomBitmap, row: bottomRow, cols: sampleCols)
    ...
    /// 获取一行的颜色采样
    private static func getRowColors(bitmap: NSBitmapImageRep, row: Int, cols: [Int]) -> [(
        r: CGFloat, g: CGFloat, b: CGFloat
    )] {
        var colors: [(r: CGFloat, g: CGFloat, b: CGFloat)] = []

        for col in cols {
            guard let color = bitmap.colorAt(x: col, y: row)?.usingColorSpace(.sRGB) else {
                continue
            }
    ...
    /// 垂直拼接两张图
    private static func mergeVertically(top: NSImage, bottom: NSImage, overlapPoints: CGFloat)
    -> NSImage {
        ...
        top.draw(
            in: CGRect(
                x: 0, y: newHeight - top.size.height, width: top.size.width, height: top.size.height
            ),
            from: .zero, operation: .sourceOver, fraction: 1.0
        )
