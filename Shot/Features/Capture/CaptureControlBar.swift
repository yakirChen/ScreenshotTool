...
final class CaptureControlBar: NSView {
    ...
    var mode: CaptureMode = .area {
        didSet {
            segmented?.selectedSegment = mode.rawValue
            updateCaptureButton()
        }
    }
    ...
    private func setupControls() {
        ...
        cancelButton.image = NSImage(
            systemSymbolName: "xmark.circle.fill", accessibilityDescription: "取消")
        ...
        segmented = NSSegmentedControl(
            images: [
                NSImage(systemSymbolName: "rectangle.dashed", accessibilityDescription: "区域")
                    ?? NSImage(),
                NSImage(systemSymbolName: "macwindow", accessibilityDescription: "窗口") ?? NSImage(),
                NSImage(systemSymbolName: "rectangle.inset.filled", accessibilityDescription: "全屏")
                    ?? NSImage()
        ...
        NSLayoutConstraint.activate([
            ...
            captureButton.leadingAnchor.constraint(
                equalTo: optionsButton.trailingAnchor, constant: 14),
    ...
    @objc private func optionsTapped() {
        ...
        let other = NSMenuItem(
            title: "其他位置…", action: #selector(chooseOtherLocation), keyEquivalent: "")
        other.target = self
        ...
        addToggleItem(
            to: menu, title: "记住上次选区", selector: #selector(toggleRememberSelection),
            isOn: prefs.rememberLastSelection)
        addToggleItem(
            to: menu, title: "显示浮动缩略图", selector: #selector(toggleFloatingThumbnail),
            isOn: prefs.showFloatingThumbnail)
        addToggleItem(
            to: menu, title: "显示鼠标指针", selector: #selector(toggleMouseCursor),
            isOn: prefs.captureMouseCursor)
        addToggleItem(
            to: menu, title: "截图后复制到剪贴板", selector: #selector(toggleClipboard),
            isOn: prefs.copyToClipboardOnCapture)
        addToggleItem(
            to: menu, title: "保存到历史", selector: #selector(toggleHistory),
    ...
    private func addLocationItem(to menu: NSMenu, title: String, path: String, current: String) {
        let item = NSMenuItem(
            title: title, action: #selector(selectLocation(_:)), keyEquivalent: "")
        item.target = self
