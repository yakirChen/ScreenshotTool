...
final class FloatingThumbnail: NSWindow {
    ...
    private func contextMenu() {
        let menu = NSMenu()
        menu.addItem(withTitle: "在编辑器中打开", action: #selector(menuOpen), keyEquivalent: "").target = self
        menu.addItem(.separator())
        menu.addItem(withTitle: "复制到剪贴板", action: #selector(menuCopy), keyEquivalent: "").target = self
        menu.addItem(withTitle: "保存到文件…", action: #selector(menuSave), keyEquivalent: "").target = self
        menu.addItem(withTitle: "钉到桌面", action: #selector(menuPin), keyEquivalent: "").target = self
        menu.addItem(.separator())
        menu.addItem(withTitle: "删除", action: #selector(menuDiscard), keyEquivalent: "").target = self
